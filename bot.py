...
MAIN_ADMIN_ID = int(os.environ.get("MAIN_ADMIN_ID", "6796088344"))
WEBHOOK_DOMAIN = os.environ.get("WEBHOOK_DOMAIN", "https://expert-octo-adventure-yhfh.onrender.com") 
...
# Global caches for ultra-fast, database-free execution during the quiz
active_votes_cache = {} 
active_quiz_tasks = {}
...
async def handle_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        ...
        try:
            await context.bot.send_message(group_id, "✅ <b>Group Approved!</b>\nGroup Admins can now use /create to set up quizzes.", parse_mode="HTML")
        except Exception:
            pass
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles deep linking logic from group /create button"""
    args = context.args
    ...
    if args and args[0].startswith("grp_"):
        try:
            ...
            logger.error(f"Deep Link Verifier Error: {e}")
            await update.message.reply_html("⚠️ <b>Error:</b> Invalid group ID or bot lacks necessary permissions.")
            return ConversationHandler.END
        ...
        text = (
            ...
            "Please provide the exact start date and time (Server Time: UTC).\n"
...
async def finalize_and_save_quiz(data, solution, admin_id, bot, context_msg):
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Start lifecycle background task for notifications and execution
...
async def load_scheduled_quizzes(bot):
    """Recover pending quizzes from DB on startup in case of restart."""
    async with AsyncSessionLocal() as session:
...
async def quiz_lifecycle_manager(quiz_id: int, bot):
    try:
        ...
        while True:
            ...
            # Exact Hour Checks (e.g. 1 hour, 2 hours remaining)
            hours_left = time_left / 3600
            nearest_hour = round(hours_left)
            milestone_key = f"hour_{nearest_hour}"
            # Notify if within 15 seconds of the exact hour mark and > 1 hour
            if nearest_hour >= 1 and abs(time_left - (nearest_hour * 3600)) <= 15 and milestone_key not in notified_milestones:
                await send_group_notif(bot, group_id, f"📢 <b>Reminder:</b>\n{notif_msg}\n\n<i>Quiz starts in exactly {nearest_hour} hours!</i>")
            ...
            for m in [15, 10, 5, 2, 1]:
                ...
                # Notify if within 15 seconds of the minute mark
                if mins_left == m and abs(time_left - (m * 60)) <= 15 and milestone_key not in notified_milestones:
                    if m > 1:
            ...
            # Sleep smartly based on time remaining to save CPU
            if time_left > 3600:
...
async def execute_quiz(quiz_id: int, bot):
    ...
    for i, q in enumerate(questions):
        ...
        # Close voting silently by removing buttons to enforce time limits
        try:
    ...
    # End processing and leaderboards
    async with AsyncSessionLocal() as session:
        quiz = await session.get(Quiz, quiz_id)
...
async def handle_vote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # SILENT SECRET RECORDING (Anti-Cheat Strategy)
    # User does NOT know if they picked the right or wrong option. 
    # This prevents using alt accounts to snipe the correct answer.
...
def main():
    ...
    # Vote handler (outside FSM so it runs globally)
    app.add_handler(CallbackQueryHandler(handle_vote, pattern="^v_"))

    # FSM Conversation Handler (FIXED)
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler("start", start_cmd)],
        states={
            START_TIME: [MessageHandler(filters.TEXT & ~filters.COMMAND, process_start_time)],
            REWARD_MSG: [MessageHandler(filters.TEXT & ~filters.COMMAND, process_reward)], # FIXED: Exclusively process_reward
    ...
    # Secure Webhook Configuration for Render
    app.run_webhook(
        listen="0.0.0.0",
        port=PORT,
        url_path=BOT_TOKEN,  
